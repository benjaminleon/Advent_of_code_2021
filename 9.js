var map = [[3, 4, 9, 8, 8, 9, 9, 0, 1, 9, 8, 7, 8, 7, 5, 4, 6, 7, 7, 8, 9, 9, 8, 7, 6, 4, 3, 4, 9, 8, 7, 8, 9, 8, 9, 1, 0, 2, 3, 4, 6, 7, 9, 9, 8, 6, 7, 7, 8, 9, 3, 2, 4, 6, 8, 9, 9, 9, 4, 1, 2, 7, 6, 7, 8, 9, 9, 9, 2, 1, 2, 4, 9, 9, 7, 8, 9, 8, 7, 5, 4, 3, 4, 5, 6, 7, 9, 9, 9, 8, 7, 8, 9, 9, 9, 8, 7, 8, 7, 6,],
...
var bottom_right = map[height - 1][width - 1];
...
for (var row = 1; row < height - 1; row++) {
    for (var col = 1; col < width - 1; col++) {
        ...
        if (point < Math.min(map[row - 1][col], map[row + 1][col], map[row][col - 1], map[row][col + 1])) {
...
for (var row = 0; row < height; row++) {
    for (var col = 0; col < width; col++) {
        ...
        var updated = update_basin(row, col);
        if (updated) {
            basin_label++;
...
console.log(basin);
var flat_basin = [].concat.apply([], basin);
var d = {};
for (var i = 0; i < basin_label; i++) {
    d[i] = 0;
}
flat_basin.forEach(function (item) { return d[item] += 1; });
d[0] = 0; // Remove edges
console.log(d);
var basin_sizes = [];
Object.keys(d).forEach(function (key) { return basin_sizes.push(d[key]); });
console.log(basin_sizes);
var product = 1;
for (var i = 0; i < 3; i++) {
    var max = Math.max.apply(Math, basin_sizes);
    product *= max;
    var index_at_max = basin_sizes.indexOf(max);
